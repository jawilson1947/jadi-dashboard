...
import { unlockUser } from "@/server/identity/service";
import { handle, ok } from "@/server/api/respond";
import { idFromUrl } from "@/server/api/params";

/** POST /api/v1/admin/users/{id}/unlock — clears an automatic lockout. */
export const POST = handle(async (req, { correlationId }) => {
  const actor = requirePermission(await getPrincipal(), "user.manage");
  return ok(await unlockUser(actor, idFromUrl(req, 2), { correlationId }), { correlationId });
});
