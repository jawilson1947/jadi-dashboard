...
| **3 — Clearance Sprint** ✅ 2026-09-18 (A-10 and A-19 signed off the same day) | Admin-entered sprint window per semester (`dash.SemesterSprint`; no computed default); `sprint.daily` job and snapshot family; page with four tabs — By date (column chart + table with total-to-date, day → students), By operator (bar + table, Operator Profiles with effective dates, Unknown/Unmapped bucket), By classification (window-scoped), Compare semesters (day-of-sprint overlay from archived snapshots, benchmarked against same-season prior semesters' nightly `tblOUSA.FinanciallyCleared` totals per A-22); Administration → Operators; dependency-free accessible SVG charts with companion tables | §7, §14.3 | Acceptance 5 (rules signed), 12; sprint totals reconcile with the hero Cleared figure |
| **3b — DNR/DNC Analysis & export core** ✅ 2026-09-18 (A-11 signed the same day) | Whole positive-balance population read once per view (`getDnrDncPopulation`, capped at 5,000) so table, footer total and export cannot disagree; two summary cards; GET-form filter bar (category, classification, balance range, last-cleared — the last narrows within the current population, it does not recompute the rule for a past term); spec default sort category → classification → last → first, kept as the tie-breaker under any column sort; total positive receivable for the filtered population; on-page documentation of the A-1 rule and the one-row-per-student key; permissioned CSV export (formula-injection safe, UTF-8 BOM, CRLF, 10,000-row cap, audited with counts and filters but never rows) | §8, §11 (export core) | Acceptance 6, 12; cards reconcile with the dashboard's DNC/DNR figures |
| **4 — Historical Analysis** ✅ 2026-09-18 (A-5, A-18, A-23 signed the same day; `docs/HISTORICAL-PLAN.md`) | `LastCleared` → `tblOUSA` semester resolver (A-22) with explicit never-cleared and unknown buckets; full-record Census and Financially-cleared trend charts (first captured semester → current, summer omitted, uncaptured terms shown as "—" not 0); academic-year view with range picker and grouped-column ⇄ line toggle; Global receivables with credit balances as a demoted secondary line (A-5), never netted; receivables by semester ⇄ school year with reconciliation lines (excluded summer terms, never cleared, unmatched codes) that make the total equal the global figure, plus aggregate CSV export; three nightly history jobs and snapshot families so trends cannot change retroactively (§9.4) | §9 | Acceptance 7; §9.3 total reconciles with §9.2 on screen and in tests |
| **5 — Student Lookup & Profile** ✅ 2026-09-24 (`docs/STUDENT-PLAN.md`; A-24 … A-29 raised the same day) | Search with min-length/wildcard guards and escaped LIKE patterns; bio card from the Bio Spec data form with the date of birth masked to its year behind `student.pii.view` and an audited reveal; authenticated photo route (A-27); both transaction cards — current semester from the co-located copy, global history behind the linked server that stays refused until A-24 is signed (D-1) — with admin-editable source labels (A-28) and per-year pagination; payment analysis with oldest-first balance aging, cash/aid ratios, and the six-month collections cue (A-25); financial clearance gate plus worksheet items and the amount needed from `fn_CostAnalysis`, with the Bio Spec's inline formula kept as a test oracle only (D-2); AI collection notice built but **disabled** pending A-13/A-26; four new permissions, six new audit actions, rate limits on search and drafting | §10, §12 | Acceptance 8, 12 |
...
| **8 — AI Analyses** | Server-side aggregate-only prompt builder, citation of metric/period/filters/snapshot, facts vs hypotheses layout, global disable, prompt/model/output audit; initial modules include **Historical trends** (reads the `historyEnrollment` / `historyReceivables` snapshots: direction and size of movement in census and clearance rate, years that break the pattern, how the receivable tracks enrolment — requested 2026-09-18) and the Clearance Sprint summary | §12 | Owner approves model hosting and data-sharing (A-13) |
...
| `OperatorProfile` | id, sourceCode (`ClearedBy`), displayName, email?, department?, isActive, isSystem (`sa` = automatic), effectiveFrom?, effectiveTo? — one row per (code, effectiveFrom), so a reused code resolves to whoever held it on the date of the action (§7.2, `db/migrations/003_sprint.sql`) |
| `SemesterSprint` | termKey, sprintStart, sprintEnd, updatedAt, updatedBy — the admin-entered sprint window (A-10). No computed default; a term without a row has no sprint (`db/migrations/003_sprint.sql`) |
...
| `ExportLog` | userId, kind (`csv`/`xlsx`/`mailmerge`), filters JSON, columns, rowCount, fileType, createdAt — **deferred to U4**: until the audit sink writes to `dash.AuditEvent`, an export is recorded as an `export.create` audit event carrying the same fields (row count, filters, column count), so there is one log to build rather than two |
...
| `GET /api/v1/dashboard/clearance-by-classification?scope=sprint\|term` | dashboard.view | rows {code, name, enrolled, cleared, notCleared, pct}, grandTotal; `sprint` counts clearance actions inside the window, `term` is the whole term |
| `GET /api/v1/dashboard/sprint/students?date=&operator=&classification=` | student.view | paginated students behind a sprint cell; 409 when no sprint window is set |
| `GET/PUT /api/v1/admin/metadata/sprint` | metadata.manage | sprint windows per semester (A-10); PUT is audited as `admin.change` |
| `GET/POST /api/v1/admin/operators` | operator.manage | operator profiles with effective dates |
| `GET /api/v1/dnr-dnc` | student.view | paginated rows (10 columns + classification name), both summary cards, filter options derived from the population, totalPositiveReceivable for the filter |
| `POST /api/v1/dnr-dnc/export` | student.view + export.create | CSV stream (`x-row-count`, `x-truncated` headers); accepts the page's form POST or JSON; writes an `export.create` audit event |
